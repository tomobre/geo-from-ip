 ...
 */
function allData(ip, noUpdate = false) {
  // console.log(`starting allData: ${ip}`)

  if (!noUpdate && (!city || !asn || Math.random() * 100 > 96)) {
    update.updateFiles()
    city = mmdbreader.openSync(cityPath)
    asn = mmdbreader.openSync(asnPath)
  }

  if (!noUpdate) console.log(`allData function called with ip: ${ip} ${noUpdate}`)
  // get all data for ip
