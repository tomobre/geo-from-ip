...
async function isRemoteNewer(database) {
  ...
  if (!fs.existsSync(mmdb)) {
    ...
    const remoteDate = new Date(response.headers.get('last-modified')).toISOString().split("T")[0];
    const localDate = new Date(stats.mtime.toUTCString()).toISOString().split("T")[0]
